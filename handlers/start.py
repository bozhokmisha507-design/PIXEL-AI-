...
OFFER_FILE_PATH = "Публичная_оферта.pdf"  # файл в корне проекта
    ...
    try:
        with open(OFFER_FILE_PATH, "rb") as f:
            await update.message.reply_document(
                document=InputFile(f, filename="Публичная_оферта.pdf"),
                caption=(
                    "📜 *Публичная оферта*\n\n"
        ...
        logger.error(f"Файл оферты не найден: {OFFER_FILE_PATH}")
...
async def show_gender_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если update содержит callback_query – редактируем сообщение
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Обработка возвратов после оплаты (для Robokassa)
    if context.args and context.args[0].startswith("payment_"):
        ...
        # ... обработка парных фото (оставьте как было)
        pass
